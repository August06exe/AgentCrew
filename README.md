...
**备份与搬家（v0.3）**：整个存档可打成单文件包带走——

```bash
python 05-scripts/save.py export                    # 存档上级目录生成 crew-save-*.asave（含全部个人数据，谨防外泄）
python 05-scripts/save.py import crew-save-*.asave  # 导入恢复：目标非空自动先备份、旧版本自动阶梯迁移，全新实例无档也可直接导入
```

管家面板「备份/恢复」页提供同款能力（导入走两步确认）。注意：lite 独立助理自带的 `_save` 不在实例包内，搬家请单独拷贝它的 `_save`。
...
| **v0.2** | 内置**理财顾问**：复式账本记账、余额与预算、微信/支付宝账单导入、月度理财简报 | ✅ |
| **v0.3** | 存档单文件导出/导入（.sav 式一键搬家） | ✅ 当前版本 |
