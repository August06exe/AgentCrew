...
### 路径 5：单文件存档包（导出/导入）——叠加功能 ✅ v0.3 已落地（2026-09-26）
`save.py export` 把 `_save\` 打成一个 `crew-save-*.asave`（像 .sav 文件），`save.py import` 读回。
- 用于备份、跨机迁移、给朋友"发个档"。建在路径 2 之上，第二期做也行，接口第一期预留。
- ✅ **已落地（2026-09-26）**：`crew-save-*.asave` 单文件包 + 四道闸（包结构/条目消毒/版本比对/自动备份）+ 原子换位；CLI 与管家面板「备份/恢复」页均可。设计定稿见 `07-ops/2026-09-26-存档导出导入设计定稿.md`。

### 最终架构：路径 2 为主体 + 路径 3 为第二层 + 路径 4 首发内建 + 路径 5 已落地
...
| 存档备份/搬家 | 拷 `_save` 文件夹 / `AGENTCREW_SAVE` 外置 / `save.py export / import`（已落地） |
...
6. **个人实例原地迁档**：现散落数据无损并入 `_save\`（全程校验行数）；
