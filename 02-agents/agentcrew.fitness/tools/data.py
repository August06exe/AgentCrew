...
def cmd_delete(a) -> int:
    ...
    return jout({"ok": True, "deleted": a.id, "trash": os.path.relpath(trash, agent_root())})
...
def cmd_update(a) -> int:
    ...
    rows[hit] = new_row
    atomic_write(tp, "".join(json.dumps(r, ensure_ascii=False) + "\n" for r in rows))
    return jout({"ok": True, "updated": a.id, "changed": list(changes), "trash": os.path.relpath(trash, agent_root())})
